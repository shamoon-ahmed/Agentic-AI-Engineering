"""
this is an asynchronous code of downloading 12 images from the internet, 
saving them locally and then processing them using an offline algorithm 
and then saving those processed images locally

now as this is an asynchronous code, images download concurrently
this saves our time a lot and makes the downloading of images concurrent
as the image processing is a CPU bound task, there's no benefit of making it async
but the way we make the CPU bound tasks faster, we add more processes 
More processes = faster CPU bound work
to implement that, we use ProcessPoolExecutor

here we're using the httpx as the alternate for requests as httpx is supported by async
so we'll get rid of the threads wrapping that we did in the previous example

and for async file operations, we'll use aiofiles package

also to limit the number of concurrent downloads, we'll use Semaphores
"""
...
# making this function async now
# this function downloads a single image and saves it locally
# then we use this function to download all 12 images by looping through the list
async def download_single_image(
    client: httpx.AsyncClient, # adding client: httpx.AsyncClient like we did in the first example using client: requests.Session 
    url: str, 
    img_num: int,
    semaphores: asyncio.Semaphore) -> Path: 
    
    async with semaphores:
        print(f"Downloading {url}...")
        ts = int(time.time())
        url = f"{url}?ts={ts}"  # Add timestamp to avoid caching issues
        response = await client.get(url, timeout=10, follow_redirects=True) # replacing requests.get() with client.get()
        response.raise_for_status()

        filename = f"image_{img_num}.jpg"
        download_path = ORIGINAL_DIR / filename

        async with aiofiles.open(download_path, "wb") as f: # using aiofiles for files operations
            async for chunk in response.aiter_bytes(chunk_size=8192): # using async for
                await f.write(chunk) # using await here

        print(f"Downloaded and saved to: {download_path}")
        return download_path

# this function downloads all 12 images using the above function by looping through the URLs list
async def download_images(urls: list) -> list[Path]: # adding async await here

    dl_semaphore = asyncio.Semaphore(DOWNLOAD_LIMIT)
    # asyncio.TaskGroup() is a modern way of running coroutines concurrently as a group like 'gather' function
    # no need to add await here as the context manager 'async with' handles it
    # using httpx client here as it supports async. so we'll get rid of the threads wrapping
    async with httpx.AsyncClient() as client:
        async with asyncio.TaskGroup() as tg:
            tasks = [
                tg.create_task(download_single_image(client, url, img_num, dl_semaphore)) # removing to_threads method
...
# then we use this function to process all 12 images by looping through the locally saved images 
...
# this function process all the 12 saved images
# we used some ProcessPoolExecutor to make more processes for the CPU to process the images faster
