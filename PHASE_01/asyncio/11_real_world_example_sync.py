"""
this is a synchronous code of downloading 12 images from the internet, 
saving them locally and then processing them using an offline algorithm 
and then saving those processed images locally

now as this is a synchronous code, images download one after another
this makes it time consuming. we dont know where the code actually takes longer
so that we can make those block of code asynchronous and optimize the code 
which makes the downloading and processing of images concurrent

to know where the code actually takes time, we can use profiling tools
tools like: scalene
scalene tells us where our code takes the most time but that doesnt mean that block of code needs to be async
the code that needs to be async is the one that does I/O bound tasks like network requests, db fetch/update, etc.
not the CPU bound tasks like image processing, etc.
we'll download this scalene tool using uv add scalene and then run a command to let scalene profile our code
command: scalene --html --outfile profile_report.html name_of_file.py 

we'll do this in the next file
"""
