...
we're converting the requests thing into threads considering that async is not supported with requests
just to see how we can run something on async as threads if async is not supported with that library
but as there's an alternate for requests library that supports async which is httpx, aiohttp,
we'll implement that in the next file
"""
...
def download_single_image(url: str, img_num: int) -> Path: # removing session: requests.Session from the arguments as we dont know if it is supported by threads
    print(f"Downloading {url}...")
    ts = int(time.time())
    url = f"{url}?ts={ts}"  # Add timestamp to avoid caching issues
    response = requests.get(url, timeout=10, allow_redirects=True) # replacing session.get() with requests.get()
